...
ترجمه فارسی (محاوره‌ای) انطباق <a href="https://github.com/kamranahmedse/design-patterns-for-humans">Design Patterns for Humans</a>  به سی شارپ C#
</h3>
<p align="center" dir="rtl"><sup>تمام توضیحات مشابه مطلب اصلی هستن و فقط مثال‌ها به زبان C# خواهند بود و باقی مطلب فقط ترجمه شده‌.<sup></p>
...
<p align="center"dir="rtl">
موضوعی که ممکنه ذهن هر کسی رو به هم بریزه. در اینجا من سعی می‌کنم این موضوع رو به طور کامل تو ذهن شما (و البته خودم) با توضیح دادن اونها به 
<i>ساده‌ترین</i>
 شکل ممکن جای بدم
</p>
<p align="center" dir="rtl">
می‌تونین مثال‌های کامل مربوط به این مقاله رو از <a href="https://github.com/anupavanm/csharp-design-patterns-for-humans-examples">اینجا</a> مشاهده کنین.
</p>
...
الگوهای طراحی راه‌حل‌هایی برای مشکلات تکراری هستن: 
<b>دستورالعمل‌هایی برای مقابله با مشکلات خاص هستن</b>.
این الگوهای طراحی یک کلاس، پکیج یا کتابخانه‌ای نیستن که بتونید اونها را به پروژه خودتون اضافه کنین و منتظر جادوشون باشین. 
در عوض، دستورالعمل‌هایی هستن در مورد چگونگی مقابله با مشکلات خاص در شرایط خاص.
</p>
...
> در مهندسی نرم‌افزار، یک الگوی طراحیِ نرم‌افزار یک راه‌حل عمومی با قابلیت استفاده مجدد
 (reusable) 
 برای یک مشکل تکراری در یک زمینه خاص از طراحی نرم‌افزاره. این الگو یک طراحی پایانی که بتونه به زبان ماشین یا سورس کد تبدیل بشه نیست؛ در حقیقت یک توضیحات یا قالبه
 (template)
 برای اینکه بدونیم یک مشکل خاص در موقعیت‌های مختلف چطوری حل می‌شه.
...
- الگوهای طراحی گوی جادویی نیستن که همه مشکلات شما را حل کنن
- سعی نکنید به زور از اونها استفاده کنین، که اگر این کار رو بکنین ممکنه اتفاقات بدی رخ بده. فراموش نکنین که الگوهای طراحی راه‌حل‌هایی 
<b>برای</b>
مشکلات هستند، نه راه‌حلی‌هایی برای 
<b>یافتن</b>
 مشکلات؛ بنابراین زیاد به مغزتون فشار نیارید.
 - اگر در زمان درست و مکان درست از اونها استفاده کنین، میتونن نجات‌دهنده باشن؛ در غیر این صورت باعث به‌هم‌ریختگی بسیار زیادی در کد خواهندشد.
 
 > همچنین توجه کنین که نمونه کدهای زیر مربوط به 
 C#-7
 است، هرچند این موضوع مانع کار یادگیری شما نمیشه چون مفاهیم همواره یک چیزه. علاوه‌بر این می‌تونین از ریپوزیتوری اصلی پیگیر باشین شاید زبان‌های دیگه هم بعدها پشتیبانی بشه.
...
به زبان ساده
> الگوهای سازنده بر روی چگونگی نمونه‌سازی (اینتنس‌گیری) یک شیء یا گروهی از اشیاء تمرکز داره.

ویکیپدیا می‌گه
> در مهندسی نرم‌افزار، الگوهای طراحی سازنده، الگوهای طراحی‌ای هستند که با مکانیزم‌های ساختن شیء کار دارن، تلاش می‌کنن اشیاء رو با روشی متناسب با بسازن. فرم ابتدایی برای ساختن یک شیء ممکنه مشکلاتی تو طراحی ایجاد کنه یا به پیچیدگی طراحی اضافه کنه. الگوهای طراحی سازنده به نوعی با کنترل این ساختن شیء این مشکل را حل کرد.

</div>

<div dir="rtl" style="text-align:right;">

> مترجم: اسامی رو بهتره با همان نام انگلیسی به خاطر بسپارید. تو اسم انواع الگوهای طراحی اسم انگلیسیشون رو هم جلوشون نوشتم ولی حس کردم بقیه بخش‌ها ترجمه کردن هم مفهوم رو نمی‌رسونه، هم یادآوریشون در آینده سخت میشه چون همه جا با همین اسامی معروف هستن
...
مثال دنیای واقعی 
> فرض کنین، شما دارین یه خونه می‌سازین و به تعدادی در نیاز دارین. خیلی بد میشه که هر زمان شما نیاز به در دارید لباس نجاریتونو بپوشید و توی خونه شروع به ساختن در کنید. به جاش شما میرید از یک کارخونه 
(factory)
اون رو می‌گیرید.
...
> الگوی طراحی 
Simple factory 
 به سادگی یک نمونه برای مشتری تولید می‌کنه بدون اینکه چیزی از منطق و روش نمونه‌سازی رو برای اون افشا کنه.

ویکیپدیا می‌گه
> در برنامه‌نویسی شیء گرا 
(OOP)
یک کارخونه، یک شیء برای ساختن بقیه اشیاست -  به طور رسمی کارخونه، یه تابع یا متده که اشیایی از کلاس‌ها و پروتوتایپ‌های مختلف را از طریق فراخوانی یک متد که فرض می‌کنیم همان 
«new»
باشه برمی‌گردونه.
...
اول از همه ما یه اینترفیسِ در داریم و پیاده‌سازی اون اینترفیس
...
بعد از اون ما یه کارخونه در داریم که اون رو می‌سازه و بر می‌گردونه
...
و به این صورت میشه ازش استفاده کرد
...
وقتی ساختن یک شیء چیزی بیشتر از یه چند تا انتساب و یه کم منطق پشتش باشه، منطقی‌تر اینه که یه کارخونه اختصاصی براش ایجاد کنیم به جای اینکه یک کد مشابه رو چند جای برنامه تکرار کنیم.
